...
__global__ void scanKernel(int* in, int n, int* out, volatile int* bSums){
    ...
    if (bSums != NULL && threadIdx.x == 0)
        bSums[blockIdx.x] = s_data[2 * blockDim.x - 1];
    __syncthreads();
    ...
    // Add block sum to local scan
    i1 = blockIdx.x * 2 * blockDim.x + threadIdx.x;
    i2 = i1 + 2 * blockDim.x;
    if (blockIdx.x > 0){
        if (i1 < n)
            out[i1] += bSums[blockIdx.x-1];
        if (i2 < n)
            out[i2] += bSums[blockIdx.x-1];
        __syncthreads();
    }
    // int j = (blockIdx.x + 1) * blockDim.x * 2 + threadIdx.x;
    // out[j] += bSums[blockIdx]
}
/*
        ...
        int useDevice=0, dim3 blkSize=dim3(1))
{
    ...
    else // Use device
    {
        ...
        if (useDevice == 1)
            ...
            scanKernel<<<gridSize, blkSize, smem>>>(d_in, n, d_out, d_blkSums);
        cudaDeviceSynchronize();
        CHECK(cudaGetLastError());

        if (gridSize.x > 1 && useDevice == 1)
        {

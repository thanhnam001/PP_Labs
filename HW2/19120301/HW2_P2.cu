        ...
        bool useDevice=false, dim3 blockSize=dim3(1), int nStreams=1)
{
	...
	else // Use device
	{
        ...
        for(int i = 0;i < nStreams;i++){
            ...
            if(i == (nStreams - 1))
                windowSize = n - p;
            size_t wBytes = windowSize * sizeof(int);
            dim3 gridSize((windowSize - 1) / blockSize.x + 1);
            CHECK(cudaMemcpyAsync(d_in1 + p, in1 + p, wBytes, cudaMemcpyHostToDevice, streams[i]));
            CHECK(cudaMemcpyAsync(d_in2 + p, in2 + p, wBytes, cudaMemcpyHostToDevice, streams[i]));
            addVecKernel<<<gridSize, blockSize, 0, streams[i]>>>(d_in1 + p, d_in2 + p, windowSize, d_out + p);
            CHECK(cudaMemcpyAsync(out + p, d_out + p, wBytes, cudaMemcpyDeviceToHost, streams[i]));
        }
        timer.Stop();

		...
		bool useDevice=false, dim3 blockSize=dim3(1))
{
	...
	else // use device
	{
		...
		uint8_t *d_in, *d_out;
		size_t nBytesIn = width * height * sizeof(uint8_t)*3;
		size_t nBytesOut = width * height * sizeof(uint8_t);
		CHECK(cudaMalloc(&d_in, nBytesIn));
		CHECK(cudaMalloc(&d_out, nBytesOut));
		// TODO: Copy data to device memories
        CHECK(cudaMemcpy(d_in, inPixels, nBytesIn, cudaMemcpyHostToDevice));
